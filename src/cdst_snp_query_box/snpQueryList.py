...
pull_risk_strat_bq_sql = """
    ...
    mp.mpr_program_status_type_desc
...
pull_scores_from_medcompass_bq_sql = """
    ...
    (
      ...
      WHERE memberriskscore_scorename in ('SNP ACCP Score', 'RAP (RRR) Score', 'DSNP_ER_3_90', 'DSNP_ER_3_90_DX', 'DSNP_ER_4_90', 'Condition Management Opportunity Score')
    ) rs
...
pull_accp_score_bq_sql = """
    SELECT 
    LEFT(CAST(member_id as string),12) as member_id,
    accp_score 
    from `anbc-hcb-prod.clin_analytics_share_hcb_prod.accp_me_shj_final`
    WHERE hospice = 0 AND dsnp_ind = 1
    """
...
# edp-prod-hcbstorage.edp_hcb_mdcrbi_ptsfty_cnsv.v_ptsfty_meas_dtl_cy_rpt is for weekly
pull_med_adherence_detail_daily_bq_sql = """
    SELECT 
        sourcememberid as src_member_id,
        measure,
        ...
        next_fill_dt,
        prescriber_name,
        precriber_phone as prescriber_phone,
        pharm_name,
        pharm_phone,
        adh_decile,
        adh_prob,
        ...
        year_end_pdc
    from `edp-prod-hcbstorage.edp_hcb_mdcrbi_ptsfty_cnsv.v_ptsfty_mapd_meas_dtl_d` daily
    left join 
    (select distinct sourcememberid, mbi from edp-prod-hcbstorage.edp_hcb_mdcrstarsdi_apps_v.Member_VW_EnterpriseIDs) id
    on daily.mbi = id.mbi
    """
...
pull_med_adherence_detail_bq_sql = """
    ...
    WHERE report_date = (select max(report_date) from `edp-prod-hcbstorage.edp_hcb_mdcrbi_ptsfty_cnsv.v_ptsfty_meas_dtl_cy_rpt`)
    """

pull_med_adherence_detail_most_recent_bq_sql = """
    SELECT * FROM (
    SELECT 
        src_member_id,
        measure,
...
pull_member_ict_case_note_bq_sql = """
    ...
    WHERE note_agg.nt_note_type_desc IS NOT NULL
    """
pull_member_ict_encounter_date_bq_sql = """
    SELECT distinct asm_id_value_desc, asm_template_nm,asm_eff_dts,asm_update_on_dts,asad_assmnt_status_typ_key_cd, asad_template_nm
    FROM `edp-prod-hcbstorage.edp_hcb_clinical_curt_cnsv.VW_LR_ASSESSMENT`
    WHERE asm_template_nm = 'ICT Encounter' and asad_assmnt_status_typ_key_cd='Completed'
    """
...
pull_ip_visit_from_hedis_bq_sql = """
    ...
    ReadmitFacility
    FROM `edp-prod-hcbstorage.edp_hcb_medbi_rptg.HEDIS_VW_PCRMemberDetail`
...
pull_home_and_community_based_services_claim_line_bq_sql = """
    with all_claim as (
    select case when left(cast(member_id as string), 2) = '10' then left(cast(member_id as string),12) else cast(member_id as string) end as src_member_id, 
    ppy.reportingyear as reportingyear,cl.prcdr_cd, cl.srv_start_dt
    from `edp-prod-hcbstorage.edp_hcb_core_cnsv.D_EMIS_CLAIM_LINE` cl join `edp-prod-hcbstorage.edp_hcb_mdcrstarsdi_apps_v.Member_VW_EnterpriseIDs_ED` eid
    on case when left(cast(member_id as string), 2) = '10' then left(cast(member_id as string),12) else cast(member_id as string) end = eid.sourcememberid join edp-prod-hcbstorage.edp_hcb_mdcrstarsdi_apps_v.Member_VW_PlanPerReportingYear ppy
    on eid.starsmember_id = ppy.starsmember_id
    where cl.business_ln_cd = 'ME'
    and cl.summarized_srv_ind='Y'
    and cl.clm_ln_status_cd='P'
    and cl.prcdr_cd in  ('G0156',
    'H0046',
    'H2017'
    'S5121',  
    'S5125',      
...
pull_skilled_nursing_facility_bq_sql = """
    select auth.mbr_identifier_value_id as src_member_id,
    line.sa_service_auth_id,
    line.sl_place_of_srvc_desc,
    line.sl_procedure_cd
    from `edp-prod-hcbstorage.edp_hcb_clinical_curt_cnsv.VW_LR_SRVC_AUTH` auth
    inner join (SELECT
    sa_service_auth_id,
    sl_place_of_srvc_desc,
    sl_procedure_cd FROM
    `edp-prod-hcbstorage.edp_hcb_clinical_curt_cnsv.SRVC_AUTH_LINE`
    WHERE (sl_place_of_srvc_desc IN ('Skilled Nursing Facility'))
    and sl_prcdr_status_desc = 'Approved'
    group by all) line
...
SELECT 
    mfb.focus_desc AS campaign,
    mfb.call_script_desc AS sub_campaign,
    mfb.mfs_cur_focus_status_reason_desc AS focus_reason_desc,
    mfb.mfs_cur_focus_status_desc AS focus_status,
    c.call_timestamp AS call_date,
    c.call_status_desc, 
    c.not_reach_reason_desc, 
    casereason.lookup_cd_display_nm AS call_disposition,
    CAST(mfb.medicare_member_id AS STRING) AS src_member_id,
    CAST(i.individual_id AS STRING) AS individual_id,
    i.first_nm, 
    i.last_nm, 
    i.birth_dt, 
    i.gender_desc, 
    ...
    i.pbp_id,
    i.lis_desc
FROM `anbc-hcb-prod.ca_mdcr_share_hcb_prod.STARS_WEB_RPT_MBR_FOCUS_BASE` AS mfb
LEFT JOIN `anbc-hcb-prod.ca_mdcr_share_hcb_prod.STARS_WEB_RPT_CALL_TRACKING_HX_BASE` AS c
    ON mfb.sgk_rpt_mbr_focus_id = c.sgk_rpt_mbr_focus_id
LEFT JOIN `anbc-hcb-prod.ca_mdcr_share_hcb_prod.STARS_WEB_RPT_SOP_LOOKUP_DATA_REF` AS casereason
    ON mfb.mfs_cur_focus_status_reason_cd_id = casereason.lookup_cd_id
    AND casereason.sgk_lookup_col_id = 37
LEFT JOIN `anbc-hcb-prod.ca_mdcr_share_hcb_prod.STARS_WEB_RPT_INDVDL_DETAILS_BASE` AS i
    ON mfb.individual_id = i.individual_id
WHERE 
    mfb.focus_cd_id IN (25)
    AND mfb.call_script_cd = 'DSNP'
    AND mfb.reporting_year IN (
...
pull_ev_call_current_year_bq_sql = """
SELECT 
    mfb.focus_desc AS campaign,
    mfb.call_script_desc AS sub_campaign,
    mfb.mfs_cur_focus_status_reason_desc AS focus_reason_desc,
...
WHERE 
    mfb.focus_cd_id IN (25)
    AND mfb.call_script_cd = 'DSNP'
    AND mfb.reporting_year = EXTRACT(YEAR FROM CURRENT_DATE())
"""


pull_ev_call_sop_bq_sql="""
SELECT 
    mfb.focus_desc AS campaign,
    mfb.call_script_desc AS sub_campaign,
    mfb.mfs_cur_focus_status_reason_desc AS focus_reason_desc,
...
LEFT JOIN `anbc-hcb-prod.ca_mdcr_share_hcb_prod.STARS_WEB_RPT_INDVDL_DETAILS_BASE` AS i
    ON mfb.individual_id = i.individual_id
WHERE mfb.call_script_cd = 'DSNP'
    AND mfb.reporting_year IN (EXTRACT(YEAR FROM CURRENT_DATE())-1, EXTRACT(YEAR FROM CURRENT_DATE()))
"""
