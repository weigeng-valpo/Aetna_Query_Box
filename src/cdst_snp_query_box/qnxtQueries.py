...
if sys.version_info[1] == 13:
    os.add_dll_directory(
        r'C:/Program Files/Python/313/Lib/site-packages/clidriver/bin')
else:
    pass
import ibm_db as db
import ibm_db_dbi
from tqdm import tqdm
from cdst_snp_query_box.select_lists import *
from cdst_snp_query_box.ServerConnection import ServerConnection
from cdst_snp_query_box import qnxtQueryList
import warnings
...
def pull_call_log_nj(call_log_date1, call_log_date2):
    ...
    query = """
                ...
                """
    row_count = pd.read_sql_query('select count(*) from ({}) subquery'.format(
        query), conn, params=[call_log_date1, call_log_date2]).iloc[0, 0]
    dfs = []
...
def pull_risk_strat_va():
    ...
    df = temp_df[['carriermemid', 'Report_Stratification']].rename(
        columns={'carriermemid': 'Member_ID', 'Report_Stratification': 'Member_risk_Stratification'})
    df['Member_risk_Stratification'] = df['Member_risk_Stratification'].map(
...
def pull_call_log_ca(ca_create_date, addl_cols=[]):
    ...
    row_count = pd.read_sql_query(
        'select count(*) from ({}) subquery'.format(query), conn, params=[ca_create_date]).iloc[0, 0]
    dfs = []
    with tqdm(total=row_count) as pbar:
...
def pull_prefer_language_qnxt():
    ...
    '''
    row_count = pd.read_sql_query(
        'select count(*) from ({}) subquery'.format(query), conn).iloc[0, 0]
    dfs = []
    with tqdm(total=row_count) as pbar:
    ...
    job_config = bigquery.QueryJobConfig(
        query_parameters=[
            bigquery.ArrayQueryParameter(
                'cob_exclusions', 'STRING', cob_exclusions),
