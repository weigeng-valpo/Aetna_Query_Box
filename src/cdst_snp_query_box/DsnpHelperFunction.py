...
"""
def _get_data_path():
    data_path = pkg_resources.resource_filename('cdst_snp_query_box', 'data')
    return data_path
...
def get_snp_contract_list(year=2026, historic=True):
    base_path = _get_data_path()
...
def get_snp_plan_list(year=2026, historic=True):
    base_path = _get_data_path()
    file_path = f'{base_path}/dsnp_contract_pbp_df.csv'
    df = pd.read_csv(file_path, dtype = {'PBP':str})
...
def get_snp_plan_state_mapping_df(year=2026, historic=False):
